...
int rp1spi_init(uint8_t spi_num, uint8_t cs_num, uint8_t mode, uint32_t freq)
{
    ...
    DEBUG_PRINT("inst->priv %p", inst->priv);
...
{
    inst->chip->interface->gpio_set_drive(inst->priv, gpio, (GPIO_DRIVE_T)1);
...
{
    inst->chip->interface->gpio_set_drive(inst->priv, gpio, (GPIO_DRIVE_T)0);
    inst->chip->interface->gpio_set_dir(inst->priv, gpio, DIR_OUTPUT);
}
