...
int rtapi_app_main(void)
{
    ...
    if ( ! checkPiType() ) {
        rtapi_print_msg(RTAPI_MSG_ERR, "Could not determine RPi type!\n");
        return -1;
    }

    if ( ! rpispi_init() ) {
        rtapi_print_msg(RTAPI_MSG_ERR,"rpispi_init failed!\n");
        return -1;
    }


    // export spiPRU SPI enable and status bits
    retval = hal_pin_bit_newf(HAL_IN, &(data->SPIenable), comp_id, "%s.SPI-enable", prefix);
